...
void SCSCL::SyncWritePos(u8 ID[], u8 IDN, u16 Position[], u16 Time[], u16 Speed[]) {
    ...
    for (u8 i = 0; i < IDN; i++) {
        ...
        Host2SCS(offbuf.data() + i * 6 + 0, offbuf.data() + i * 6 + 1, Position[i]);
        Host2SCS(offbuf.data() + i * 6 + 2, offbuf.data() + i * 6 + 3, T);
        Host2SCS(offbuf.data() + i * 6 + 4, offbuf.data() + i * 6 + 5, V);
    }
    syncWrite(ID, IDN, SCSCL_GOAL_POSITION_L, offbuf.data(), 6);
}
